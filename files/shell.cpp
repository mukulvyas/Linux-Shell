...
bool containsHelpOption(const std::vector<std::string>& tokens) {
    ...
    if (tokens.size() > 1) {
        ...
        for (size_t i = 2; i < tokens.size(); ++i) {
            if (tokens[i] == "-L") {
                ...
                std::cerr << "Unknown option: " << tokens[i] << std::endl;
                return;
            }
        }
        ...
        if (followSymlinks) {
            if (fs::exists(targetPath) && fs::is_directory(targetPath)) {
                ...
                std::cout << "Directory not found: " << targetPath << std::endl;
            }
        } else if (usePhysicalStructure) {
            try {
                fs::current_path(fs::canonical(targetPath));
            } catch (const fs::filesystem_error& e) {
                std::cerr << "Error determining physical directory structure: " << e.what() << std::endl;
    ...
    for (size_t i = 1; i < tokens.size(); ++i) {
        if (tokens[i] == "-a" || tokens[i] == "--all") {
            ...
            std::cout << "Unknown option: " << tokens[i] << std::endl;
            return;
        }
    }
    ...
    if (showCount) {
        std::cout << "\nTotal: " << fileNames.size() << " items" << std::endl;
    }
...
void listSubdirectoryContents(const std::string& directory,
    ...
    std::cout << "\n" << directory << ":\n";
    listDirectoryContents(std::vector<std::string>{ "-a", "-c", "-r", "-R" });
    fs::current_path("..");
}
...
void moveFile(const std::vector<std::string>& tokens, const std::string& command) {
    ...
    if (tokens.size() < 3) {
        std::cerr << "Usage: mv [options] source destination" << std::endl;
        return;
    }

    // Construct the base "mv" command
    std::string mvCommand = "mv";
    int result;
...
void copyFile(const std::vector<std::string>& tokens,const std::string& command) {
    ...
    for (size_t i = 1; i < tokens.size(); ++i) {
        if (tokens[i] == "-r" || tokens[i] == "--recursive") {
            recursiveCopy = true;
        } else if (tokens[i] == "-f" || tokens[i] == "--force") {
            forceOverwrite = true;
    ...
    void displayHelp(const std::string& command) {
       ...
       if (command == "cd") {
        ...
        cout << "    -P is used; non-zero otherwise.\n";      
            ...
            std::string output = executeCommand("ls --help");
            cout << "\nOutput:\n" << output << endl;
        } else if (command == "mv") {
            std::string output = executeCommand("mv --help");
            cout << "\nOutput:\n" << output << endl;
            
        } else if (command == "rm") {
            std::string output = executeCommand("rm --help");
            cout << "\nOutput:\n" << output << endl;
            
        } else if (command == "cp") {
            std::string output = executeCommand("cp --help");
            cout << "\nOutput:\n" << output << endl;
